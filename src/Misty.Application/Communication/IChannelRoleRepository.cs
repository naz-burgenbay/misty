...
public interface IChannelRoleRepository
{
    ...
    Task<IReadOnlyList<ChannelRole>> GetByChannelIdAsync(Guid channelId, CancellationToken ct = default);
    Task AddAsync(ChannelRole role, CancellationToken ct = default);
    Task UpdateAsync(ChannelRole role, byte[] concurrencyToken, CancellationToken ct = default);
